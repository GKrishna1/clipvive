...
def login(payload: LoginIn, session=Depends(get_session)):
    ...
    # session enforcement: allow at most one active session per device_type
    ...
    if len(existing) >= 1:
        # revoke the oldest session of this device_type
...
# new: list sessions for logged-in user
...
def list_sessions(user=Depends(lambda authorization=None, session=Depends(get_session): get_current_user(authorization, session)), session=Depends(get_session)):
    q = select(UserSession).where(UserSession.user_id == user.id)
    sessions = session.exec(q).all()
    out = []
    for s in sessions:
        out.append(SessionOut(id=s.id, device_id=s.device_id, device_type=s.device_type, jti=s.jti, created_at=s.created_at))
    return out

# new: revoke session by DB id (owner only)
@router.delete("/sessions/{session_db_id}")
def revoke_session(session_db_id: int = Path(..., gt=0), user=Depends(lambda authorization=None, session=Depends(get_session): get_current_user(authorization, session)), session=Depends(get_session)):
    s = session.get(UserSession, session_db_id)
    if not s or s.user_id != user.id:
        raise HTTPException(status_code=404, detail="Session not found")
    session.delete(s)
    session.commit()
    return {"revoked": True, "session_id": session_db_id}

# dependency reused by other modules
